#!/usr/bin/env python3
"""
Fix Overlay Positioning and Opacity
MTG Quiz App - Adjust name overlay to cover only the name area precisely

Fixes:
1. Position overlay correctly over just the name (not mana cost or artwork)
2. Make overlay completely opaque (no transparency)
3. Adjust size to match actual name area dimensions

Run from project root: python python/fix_overlay_positioning_opacity.py
"""
...
def update_card_guessing_game_component():
    """Update CardGuessingGame with precise name overlay positioning"""
              ...
              <div className="relative inline-block">
                ...
                {!gameState.isGuessSubmitted && (
                  <div 
                    className="absolute"
                    style={{
                      // Precise positioning over JUST the name area (lower portion of card)
                      top: '68%',     // Much lower - name is below artwork and type line
                      left: '7%',     // Small left margin
                      right: '35%',   // Leave room for mana cost on right
                      height: '5.5%', // Much smaller height - just name area
...
def main():
    ...
    print("🎯 Fixing Overlay Positioning and Opacity")
    print("=" * 45)
    ...
    try:
        ...
        print("   • Repositioned overlay to cover ONLY the name area (68% from top)")
        print("   • Reduced height to 5.5% (much smaller)")
        print("   • Adjusted left/right margins to avoid mana cost")
        print("   • Made overlay completely opaque (opacity: 1)")
        print("   • Added subtle border and shadow for blending")
    except Exception as e:
        print(f"❌ Error updating CardGuessingGame.tsx: {e}")
        return 1
    
    print("\n" + "=" * 45)
    print("✅ SUCCESS! Overlay is now precisely positioned and opaque.")
    print("\n🎯 The overlay now covers only the card name area!")
    print("   • Mana cost visible in top right")
    print("   • Artwork completely unobstructed")
    print("   • Name area cleanly hidden")
    print("   • Completely opaque overlay")
