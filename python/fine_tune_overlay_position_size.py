#!/usr/bin/env python3
"""
Fine Tune Overlay Position and Size
MTG Quiz App - Move overlay higher and make it longer to fully cover card names

Adjustments based on user feedback:
1. Move overlay higher up (reduce top percentage)
2. Make overlay longer (reduce right margin)
3. Keep same height and opacity

Run from project root: python python/fine_tune_overlay_position_size.py
"""
...
def update_card_guessing_game_component():
    """Update CardGuessingGame with precise overlay positioning"""
              ...
              <div className="relative inline-block">
                ...
                {!gameState.isGuessSubmitted && (
                  <div 
                    className="absolute"
                    style={{
                      // Fine-tuned positioning to fully cover name
                      top: '5.5%',    // Moved up higher from 8%
                      left: '7%',     // Same left margin
                      right: '25%',   // Extended longer (was 45%, now 25%)
                      height: '5.5%', // Keep same height
...
def main():
    ...
    print("🎯 Fine Tuning Overlay Position and Size")
    print("=" * 42)
    ...
    try:
        ...
        print("   • Moved overlay higher (5.5% from top, was 8%)")
        print("   • Made overlay longer (right: 25%, was 45%)")
        print("   • Keeps same height and opacity")
        print("   • Should now fully cover long card names")
    except Exception as e:
        print(f"❌ Error updating CardGuessingGame.tsx: {e}")
        return 1
    
    print("\n" + "=" * 42)
    print("✅ SUCCESS! Overlay positioning fine-tuned.")
    print("\n🎯 The overlay should now perfectly cover card names!")
    print("   • Higher positioning to match name location")
    print("   • Extended length for longer names")
    print("   • Still avoids mana cost area")
